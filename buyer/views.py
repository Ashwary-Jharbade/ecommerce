...
def buyerapp(request):
    ...
    if request.method == 'POST' and request.POST['ser']:
        ...
        p=Products.objects.filter(category__cat=req)
        for i in p:
            count+=1
...
def proddetails(request,id):
    ...
    return render(request,'viewproduct.html',{'pro':p})
...
def cartitems(request):
    ...
    return render(request,'showcart.html',{'cart':car,'total':total,'count':count})
...
def addDeliveryDetails(request,id):
    ...
    return render(request,'prodDeliveryDetails.html',{'address':uobj,'product':querryprod,'idy':id})
...
def addAddress(request):
    ...
    if request.method == 'POST':
        fom = userAddressForm(request.POST)
        fom.save()
        return redirect('/buyer/buyerprofile/')
